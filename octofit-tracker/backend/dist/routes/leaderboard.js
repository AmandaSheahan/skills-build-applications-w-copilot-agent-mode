...
router.get('/', async (_request, response) => {
    try {
        ...
        response.status(500).json({ error: 'Failed to fetch leaderboard', details: error.message });
    }
});
router.post('/', async (request, response) => {
    try {
        ...
        response.status(400).json({ error: 'Failed to create leaderboard entry', details: error.message });
    }
});
